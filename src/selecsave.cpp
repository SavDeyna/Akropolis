...
void SelecSave::setupUI() {
    ...
    // Activer les boutons seulement si une ligne est cliquée
    connect(m_listSaves, &QListWidget::itemClicked, [this]() {
        m_btnCharger->setEnabled(true);
        m_btnSuppr->setEnabled(true);
    });

    // Connexion du bouton Charger
    connect(m_btnCharger, &QPushButton::clicked, [this]() {
        QListWidgetItem* item = m_listSaves->currentItem();
        if (item) {
            int idSave = item->data(Qt::UserRole).toInt();
            emit saveSelected(idSave);
        }
    });

    // Connexion du bouton Supprimer
    connect(m_btnSuppr, &QPushButton::clicked, [this]() {
        QListWidgetItem* item = m_listSaves->currentItem();
        if (item) {
            int idSave = item->data(Qt::UserRole).toInt();
            emit saveDeleted(idSave);
        }
    });
}
